...
- **plano de datos**: ejecuta el proceso de reenvío mediante hardware especializado para máxima eficiencia. La parte de puertos de entrada, puertos de salida y fabric de conmutación formarían parte del plano de control.
...
<!-- TODO: ESTO ESTÁ MAL. -->
Esta división entre parte de red y parte del host permite representar de forma jerárquica la estructura de direccionamiento como se muestra en la @fig-example-network. Los routers pueden tomar decisiones de reenvío basándose únicamente en la parte de red de la dirección destino, consultando sus tablas de enrutamiento locales para determinar hacia qué interfaz dirigir el paquete. Esto permite que cada router mantenga información agregada sobre redes remotas sin necesidad de conocer cada host individual, reduciendo drásticamente el tamaño de las tablas de enrutamiento y haciendo escalable la infraestructura global de Internet. 
...
La estructura de direccionamiento IPv4, que permite distinguir entre red y host para generar una arquitectura jerárquica de redes, inicialmente utilizaba un sistema. En este sistema de clases, las direcciones IPv4 se categorizaban en tres grupos principales según los bits iniciales del primer octeto, determinando la división entre bits de red y host. La Clase A comenzaba con bit 0, la Clase B con bits "10", y la Clase C con bits "110", creando saltos enormes entre las capacidades de cada categoría que generaban ineficiencias significativas en la asignación.
